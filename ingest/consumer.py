...
def write_batch(con: duckdb.DuckDBPyConnection, batch: list[dict]) -> None:
    ...
    Replay cùng một event_id phải thay thế payload cũ: consumer có thể chết
    sau khi ghi nhưng trước khi commit offset, rồi đọc lại cả batch đó.
    """
    con.executemany(
        f"""
...
def consume(
    ...
    with LogConsumer(topic, offset_file) as consumer:
        ...
        while True:
            ...
            consumer.commit()                 # chỉ commit sau khi đã ghi
            # ─────────────────────────────────────────────────────────────
